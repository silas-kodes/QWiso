...
import { z } from 'zod';
import { createDataset, getDataset, getAllDatasets, deleteDataset, createNumbersBatch, getNumbersByDataset, getNumbersCountByDataset, resetDatasetValidationStatus, isNumberBlacklisted, } from '../db/queries.js';
...
router.post('/generate', (req, res) => {
    ...
    try {
        ...
        // Create dataset
        const datasetId = createDataset(`${country.flag} ${country.name} - ${options.quantity} numbers`, country.code, country.name, country.dial, options.quantity, options);
        // Store numbers
...
router.get('/:id/numbers', (req, res) => {
    ...
    const numbers = getNumbersByDataset(id, status, parseInt(limit), parseInt(offset));
    res.json(numbers);
});
// Delete dataset
