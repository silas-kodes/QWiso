...
async function runVerification() {
  ...
  const datasetId = createDataset(
    "🧪 Integration Test Verification Dataset",
    "US",
    "United States",
    "+1",
    options.quantity,
  ...
  batch1.forEach(num => {
    ...
    if (status === 'invalid') batch1InvalidCount++;
    else batch1ValidCount++;
  ...
  if (
    ...
    throw new Error(
      `🚨 FAILURE: Mismatched counts in database! Expected total: ${expectedTotal}, got: ${countsMiddle?.total}. Expected valid: ${expectedValid}, got: ${countsMiddle?.valid}. Expected invalid: ${expectedInvalid}, got: ${countsMiddle?.invalid}`
    );
  }
  ...
  const resumedProcessedIds: string[] = []; // empty, representing a fresh process boot
