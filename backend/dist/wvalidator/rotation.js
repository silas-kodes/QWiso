...
function computeHealth(s) {
    const now = Date.now();
    if (s.checksThisSession >= ROTATION_RULES.MAX_CHECKS_PER_SESSION)
        return 'exhausted';
    ...
    if (s.recentResults.length >= 5) {
        ...
        if (rate >= ROTATION_RULES.UNHEALTHY_ERROR_RATE_THRESHOLD)
            return 'degraded';
    }
    return 'healthy';
}
function broadcastHealth(s) {
    broadcastToClients({
...
function recordResult(s, success) {
    ...
    if (s.consecutiveErrors >= ROTATION_RULES.CONSECUTIVE_ERROR_THRESHOLD) {
        const cooldownMs = Math.min(ROTATION_RULES.BASE_COOLDOWN_MS * Math.pow(2, s.cooldownCount), ROTATION_RULES.MAX_COOLDOWN_MS);
        s.cooldownUntil = Date.now() + cooldownMs;
        s.cooldownCount++;
        s.consecutiveErrors = 0; // reset so it can re-accumulate after cooldown
        console.warn(`[Rotation] Account ${s.name} (${s.id}) entering cooldown for ${cooldownMs / 1000}s ` +
...
export function pickNextAccount() {
    ...
    const eligible = readyInstances
        ...
        if (s.cooldownUntil > now)
            return false;
        if (s.checksThisHour >= ROTATION_RULES.MAX_CHECKS_PER_HOUR)
            return false;
        return true;
    });
 ...
 */
export async function rotatedCheck(digits, timeoutMs = 30_000) {
    ...
    try {
        const result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout checking ${digits}`)), timeoutMs)),
        ]);
        valid = result;
 ...
 */
export function resetAccountSession(id) {
 ...
 */
export function forceAccountCooldown(id, durationMs) {
    const manager = getWhatsAppManager();
