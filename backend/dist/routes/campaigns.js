...
import { createCampaign, getCampaigns, getCampaign, updateCampaignStatus, getValidCountForDataset, deleteCampaign } from '../db/queries.js';
...
router.post('/:id/start', (req, res) => {
    ...
    // Pre-check: warn if no valid numbers
    const validCount = getValidCountForDataset(campaign.dataset_id);
    if (validCount === 0) {
        res.status(400).json({
    ...
    jobQueue.enqueue(id, async (token) => {
        await runCampaignJob(id, token);
    });
    res.json({ success: true, message: 'Campaign started', validContacts: validCount });
});
